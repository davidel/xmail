...
are supplied inside the "win64ssl" directory of the XMail source package.
...
inside the 'B<win64ssl\conf>' directory of the source package.
...
'B<c_rehash.pl>' is contained inside the 'B<win64ssl\bin>' subdirectory of the source package).
Unix users will find proper CA certificates inside the standard install paths of OpenSSL,
while Windows users will find them inside the 'B<win64ssl\certs>' subdirectory of the
